...
public abstract class GameObject {
	...
	public Rectangle getBounds(Side s) {
		int vSpeed = (int)(1 * 0.8f);
		int hSpeed = (int)(1 * 0.8f);
		switch (s) {
		case top:
			return new Rectangle((int)x,                      (int)y - Math.abs(vSpeed),   (int)width,              (int)Math.abs(vSpeed));
		case bottom:
			return new Rectangle((int)x,                      (int)y + (int)height,        (int)width,              (int)Math.abs(vSpeed));
		case left:
			return new Rectangle((int)x - Math.abs(hSpeed),   (int)y,                      (int)Math.abs(hSpeed),   (int)height);
		case right:
			return new Rectangle((int)x + (int)width,         (int)y,                      (int)Math.abs(hSpeed),   (int)height);
		default:
